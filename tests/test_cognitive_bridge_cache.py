...
def test_healing_cache_persistence_roundtrip(tmp_path: Path, monkeypatch):
    cache_file = tmp_path / "healing_cache.json"

    monkeypatch.setattr(CognitiveBridge, "_cache_file", cache_file)
    monkeypatch.setattr(CognitiveBridge, "_cache_loaded", False)
    monkeypatch.setattr(CognitiveBridge, "_healing_cache", {})
